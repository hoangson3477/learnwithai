  ...
  const fetchUserStats = useCallback(async () => {
    try {
      ...
      const { data: userData } = await supabase
        .from('users')
        .select('total_quizzes_taken, total_chat_sessions, learning_topics')
        .eq('id', user?.id)
        .single();
      ...
      <div className="max-w-7xl mx-auto p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <h3 className="text-gray-500 text-sm font-semibold">Bài thi đã làm</h3>
            <p className="text-3xl font-bold text-gray-800">{stats.totalQuizzesTaken}</p>
          </div>

          <div className="bg-white rounded-lg shadow p-6">
            <div className="text-4xl text-green-600 mb-2">💬</div>
            <h3 className="text-gray-500 text-sm font-semibold">Phiên chat</h3>
            <p className="text-3xl font-bold text-gray-800">{stats.totalChatSessions}</p>
          </div>

          <div className="bg-white rounded-lg shadow p-6">
            <div className="text-4xl text-purple-600 mb-2">🎓</div>
            <h3 className="text-gray-500 text-sm font-semibold">Chủ đề đang học</h3>
            <p className="text-3xl font-bold text-gray-800">{stats.learningTopics}</p>
          </div>

          <div className="bg-white rounded-lg shadow p-6">
            <div className="text-4xl text-orange-600 mb-2">🔥</div>
            <h3 className="text-gray-500 text-sm font-semibold">Chuỗi học tập</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          >
            ...
            <h2 className="text-xl font-bold text-gray-800 mb-2">Bắt đầu chat</h2>
            <p className="text-gray-600">Học từ các trợ lý AI</p>
          </Link>
          ...
          >
            <div className="text-5xl mb-3">📚</div>
            <h2 className="text-xl font-bold text-gray-800 mb-2">Làm bài thi</h2>
          ...
          >
            <div className="text-5xl mb-3">📖</div>
            <h2 className="text-xl font-bold text-gray-800 mb-2">Duyệt tài liệu</h2>
            <p className="text-gray-600">Truy cập tài liệu học tập</p>
          </a>
        </div>
        ...
        {/* Learning Materials */}
        <div id="documents" className="bg-white rounded-lg shadow">
          <div className="p-6 border-b border-gray-200">
            <h2 className="text-2xl font-bold text-gray-800">Tài liệu được đề xuất</h2>
          </div>
          ...
          {documents.length === 0 ? (
            ...
            <div className="divide-y divide-gray-200">
              {documents.slice(0, 5).map((doc) => (
                <div key={doc.id} className="p-6 hover:bg-gray-50 transition cursor-pointer">
